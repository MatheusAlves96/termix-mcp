...
| `TERMIX_TOOLSETS`          | no       | `default`           | Comma-separated toolset ids, or `all`. See [available tools](#available-tools-resources-and-prompts). |
...
Tools are grouped into toolsets you can enable independently (`TERMIX_TOOLSETS`, see
[Configuration](#configuration)). What's intentionally not exposed — login/OIDC/WebAuthn flows,
interactive terminal/RDP over WebSocket, etc. — is explained in [ARCHITECTURE.md](ARCHITECTURE.md).

Run `npx termix-mcp --print-tools` to see the exact set of tools active for your configuration and
detected Termix version.

Everything below this line, down to the end marker, is generated by `npm run spec:inventory` straight
from `src/catalog`, `src/resources`, and `src/prompts` — not hand-maintained. CI (`spec:check-docs`) fails
the build if this section falls out of date with the code, so it can't silently drift.

<!-- GENERATED:TOOLS-RESOURCES-PROMPTS:START -->

416 operations total, 392 exposed as tools, 24 intentionally not exposed (see reasons below).
...
| ------------------------ | ---------------- | ---------------------------------------------------- | ------------------------------------------ |
| `termix://system/status` | Termix status    | Health and version of the connected Termix instance. | _(aggregates /health + /version directly)_ |
| `termix://hosts`         | SSH hosts        | Every SSH host configured in Termix.                 | `termix_hosts_get_all_ssh_hosts`           |
| `termix://snippets`      | Command snippets | Every saved command snippet.                         | `termix_snippets_get_all_snippets`         |
| `termix://fleets`        | Fleets           | The current user's fleets (grouped hosts).           | `termix_fleets_list_current_users_fleets`  |
| `termix://hosts/status`  | Host statuses    | Live status for every monitored host.                | `termix_metrics_get_all_host_statuses`     |
...
<!-- GENERATED:TOOLS-RESOURCES-PROMPTS:END -->
