...
| `npm run spec:inventory`                    | Regenerate README.md's Tools/Resources/Prompts section from the catalog.                           |
| `npm run spec:check-docs`                   | What CI runs: fails if that section is stale. Run this, not just `spec:inventory`, before pushing. |
...
3. Run `npm run spec:inventory` and commit the regenerated section of README.md. CI runs `spec:check-docs`
   and fails the build if you forget - the same applies to adding/changing a resource or a prompt.
...
- Update the relevant root-level doc (`README.md`, `CONFIGURATION.md`, `COMPATIBILITY.md`) in the same PR
